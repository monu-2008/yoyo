...
export async function GET() {
  try {
    const appName = "seed-" + Date.now();
    const app = initializeApp(firebaseConfig, appName);
    const db = getDatabase(app);

    // Force re-seed always (for fresh data)
    const now = Date.now();
    ...
    // 3. Staff members (for staff login - email/password checked against RTDB)
    await set(ref(db, "staff"), {
      ...
      staff_4: {
        ...
        active: false, // Disabled staff member
        createdAt: now - 86400000 * 5,
      },
    });

    // 4. Sample service requests (various statuses)
    ...
    // 6. Sample enquiries (from Contact form)
    ...
    return NextResponse.json({
      message: "Firebase seeded successfully with complete data!",
      status: "ok",
      data: {
        products: 12,
        staff: 4,
        ...
        productOrders: 4,
        enquiries: 2,
        adminCredentials: { email: "admin@racecomputer.in", password: "admin123" },
        staffLogins: [
          { email: "rahul@racecomputer.in", password: "rahul123", active: true },
          { email: "amit@racecomputer.in", password: "amit123", active: true },
          { email: "vikram@racecomputer.in", password: "vikram123", active: true },
          { email: "priya@racecomputer.in", password: "priya123", active: false },
        ],
      }
    });
  } catch (error: unknown) {
    ...
    return NextResponse.json({
      error: msg,
      hint: "If you see PERMISSION_DENIED, please update your Firebase Realtime Database rules to allow read/write access. Go to Firebase Console > Realtime Database > Rules and set: { \"rules\": { \".read\": true, \".write\": true } }"
    }, { status: 500 });
  }
}
