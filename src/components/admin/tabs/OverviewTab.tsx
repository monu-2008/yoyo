...
import {
  Wrench, ShoppingBag, Users, Package,
  CheckCircle, Clock, AlertCircle,
  TrendingUp, BarChart3,
} from "lucide-react";
import type { ServiceRequest, ProductOrder, StaffMember, Product } from "@/lib/adminTypes";
            ...
            {pendingRequests.length === 0 ? (
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {pendingRequests.slice(0, 5).map((r) => (
                  <div key={r.id} className={`flex items-center justify-between p-3 rounded-lg border`} style={masterVerified ? { background: "rgba(250,204,21,0.1)", border: "1px solid rgba(250,204,21,0.15)" } : { background: "rgba(250,204,21,0.05)", border: "1px solid #fef3c7" }}>
                    <div>
                      <div className={`text-sm font-semibold ${masterVerified ? "text-gray-200" : "text-gray-900"}`}>{r.name}</div>
                      <div className={`text-xs text-gray-500`}>{r.serviceType} · {formatDate(r.createdAt)}</div>
                    </div>
            ...
            {productOrders.length === 0 ? (
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {productOrders.slice(0, 5).map((o) => (
                  <div key={o.id} className={`flex items-center justify-between p-3 rounded-lg border`} style={masterVerified ? { background: "rgba(0,229,255,0.08)", border: "1px solid rgba(0,229,255,0.15)" } : { background: "rgba(239,68,68,0.05)", border: "1px solid #fecaca" }}>
                    <div>
                      <div className={`text-sm font-semibold ${masterVerified ? "text-gray-200" : "text-gray-900"}`}>{o.productName}</div>
                      <div className={`text-xs text-gray-500`}>{o.customerName} · {formatDate(o.createdAt)}</div>
                    </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {staff.filter(s => s.active !== false).map((s) => {
                ...
                return (
                  <div key={s.id} className={`flex items-center justify-between p-3 rounded-lg border`} style={masterVerified ? { background: "rgba(124,58,255,0.08)", border: "1px solid rgba(124,58,255,0.15)" } : { background: "rgba(239,68,68,0.05)", border: "1px solid #fecaca" }}>
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs`} style={masterVerified ? { background: "rgba(0,229,255,0.15)", color: "#00e5ff" } : { background: "#fef2f2", color: "#dc2626" }}>{s.name.charAt(0).toUpperCase()}</div>
                      <div>
                    ...
                    <div className="flex items-center gap-3">
                      <Badge className={`text-[10px]`} style={masterVerified ? { background: "rgba(0,229,255,0.15)", color: "#00e5ff", border: "1px solid rgba(0,229,255,0.2)" } : { background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca" }}>{stats.active} Active</Badge>
                      <Badge className={`text-[10px]`} style={masterVerified ? { background: "rgba(16,185,129,0.15)", color: "#10b981", border: "1px solid rgba(16,185,129,0.2)" } : {}}>{stats.completed} Done</Badge>
                    </div>
                  </div>
                );
              })}
